...
if True:
    stroka_basa += str(cto_ti) + "/" + str(login) + "/" + str(parol) + "/" # 0, 1, 2
    stroka_basa += user_pred + "/" + parolchik + "/" + user + "/" # 3, 4, 5
    stroka_basa += str(login_vvod) + "/" + str(parol_vvod) + "/" + user_parol + "/" + str(registr) # 6, 7, 8, 9
    stroka_basa += "/" + str(level) + "/" + str(exp) # 10, 11
    stroka_basa += "/" + str(user_hp)+ "/" + str(user_damage) + "/" + str(user_crit_damage) + "/" + str(user_chance_crit) # 12, 13, 14, 15
    stroka_basa += "/" + mob + "/" + str(hp_mob) + "/" + str(max_hp) + "/" + inventory + "/" + str(points) # 16, 17, 18, 19, 20
    stroka_basa += "/" + str(sword) + "/" + str(sword_char) # 21, 22
    stroka_basa += "/" + str(brona) + "/" + str(brona_char) # 23, 24
    stroka_basa += "/" + str(artefact) + "/" + str(artefact_char) + "/" + predmet_craft + "/" + inventory_swords + "/" + str(chari) + "/" + str(real_damage) + "/" + str(zel)+ "/" + str(location) # 25 - 32
    stroka_basa += "/" + str(shield) + "/" + str(vip) # 33, 34
...
based_packs_loot = {
    "🧬 characteristic pack 🧬": {"type": "default",
            "loot1": "☠ череп ☠", "loot1_chance": "40%", "loot1_count": "10",
            "loot3": "⚪ белая сфера ⚪", "loot3_chance": "30%", "loot3_count": "1",
            "loot2": "🃏 игральный камень 🃏", "loot2_chance": "20%", "loot2_count": "1",
            "loot5": "💧 шкура водяного змея 🐍", "loot5_chance": "9%", "loot5_count": "1",
            "loot4": "point", "loot5_chance": "1%", "loot4_count": "1"
    }
}

based_packs_cost = {
    "🧬 characteristic pack 🧬": {"type": "default",
            "loot1": "🃏 игральный камень 🃏", "loot1_count": "1",
    }
}

# словарь врагов в локации пустошь
based_monstrs_of_pystosh_vvod = {
                "🖤 смерть 🖤  (Lvl #?%$)": {"type": "default", "hp": 99999, "damage": 9999, "exp": 999999}
}
...
based_monstrs_of_prosstoles_loot = {
                ...
                "🖤 смерть 🖤  (Lvl #?%$)": {"type": "default",
                        "loot1": "☠ череп ☠", "loot1_chance": "100%", "loot1_count": "70-100",
                        "loot2": "💜 темная энергия 💜", "loot2_chance": "10%", "loot2_count": "1-2",
                        "loot3": "🖤 $3&#9\%#4? 🖤", "loot3_chance": "0.00001%", "loot3_count": "1-1"
                }
}
...
posledovat = ["🦎 хвост ящерицы 🦎", "🍈 чешуя (F) 🍈", "🃏 игральный камень 🃏", "🌰 орех 🌰", "🐿 шкура белки 🐿", "⛓ кусочек цепи ⛓", "💰 мешок монет 💰",
                ...
                "💀 змеиный клык 🐍", "💧 шкура водяного змея 🐍", "🌀 руна водяного змея 🌀", "🍈 чешуя (F+) 🍈", "💜 темная энергия 💜", "🖤 $3&#9\%#4? 🖤", "☠ череп ☠"
]
...
def start(message):
    ...
    f = open("tg_bot/f.txt", encoding="utf8", mode = 'r')
...
def start(message):
    ...
    markup.add(btn2)
    # вывод характеристик
    s = "Статистика игрока:" + "\n" + "\n" + f"✅ Lvl: {level}" + "\n" + f"✅ Exp: {expa}/{level * 100}" + "\n" + "\n" + f"❤ Hp: {hp}/{max_h}" + "\n" + f"🛡️ Defence: {defence}" + "\n"
    s += f"⚔️ Damage: {damage}" + "\n" + "\n" + f"💥 Crit chance: {chance}%" + "\n" f"💥 Crit damage: {100 + crit}%" + "\n" + "\n" + f"🍀 Free points: {point}"
    bot.send_message(message.chat.id, s, reply_markup=markup)
...
def start(message):
    ...
    if search == 0:
        ...
        f = open("tg_bot/f.txt", encoding="utf8", mode = 'w')
...
def func(message):
    ...
    if ab[9] == "0":
        ...
        # конец регистрации
            ...
            stroka_basa += "/" + str(ab[33])+ "/" + str(ab[34])
        ...
        # главное меню
            ...
            btn10 = types.KeyboardButton("☯️ пустошь ☯️")
            btn11 = types.KeyboardButton("🃏 packs 🃏")
            markup.add(btn1, btn9, btn10)
            markup.add(btn6, btn8)
        ...
        #локация 4
            ...
            if ab[34] == "100":
                btn3 = types.KeyboardButton("🎭 подойти к сомнительному магазинчику 🎭")
            btn4 = types.KeyboardButton("назад")
            markup.add(btn1, btn2)
            if ab[34] == "100":
                markup.add(btn3)
            ...
            bot.send_message(message.chat.id, f"у вас вычтенно: {t}exp", reply_markup=markup)
            ...
            if ab[11] == "0":
                bot.send_message(message.chat.id, "⚠️ на балансе 0exp ⚠️", reply_markup=markup)
                bot.send_message(message.chat.id, "такими темпами вы не поднимите уровень...", reply_markup=markup)
            if ab[34] == "100":
                bot.send_message(message.chat.id, "...", reply_markup=markup)
                bot.send_message(message.chat.id, "однако...", reply_markup=markup)
                bot.send_message(message.chat.id, "любопытность порой приносит свои плоды...", reply_markup=markup)
                bot.send_message(message.chat.id, "вы нашли скрытую лавку", reply_markup=markup)
                bot.send_message(message.chat.id, "🎭", reply_markup=markup)
        ...
        elif message.text == "🎭 подойти к сомнительному магазинчику 🎭" and ab[34] == "100":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn3 = types.KeyboardButton("🎭 уйти 🎭")
            btn1 = types.KeyboardButton("👑 взять билет 👑")
            markup.add(btn1)
            markup.add(btn3)
            bot.send_message(message.chat.id, "вы погружаетесь в пучину 🎭", reply_markup=markup)
            bot.send_message(message.chat.id, "это ваш первый шаг на этом пути бесчестия...", reply_markup=markup)
            bot.send_message(message.chat.id, "не дайти алчности захватить себя 💔", reply_markup=markup)
            bot.send_message(message.chat.id, "вы нашли какойто билетик...", reply_markup=markup)
            vipstr = "--------------" + "\n" + "👑 vip 👑" + "\n" + "--------------"
            bot.send_message(message.chat.id, vipstr, reply_markup=markup)
            bot.send_message(message.chat.id, "вы читаете надпись на обороте, написанную мелким шрифтом:", reply_markup=markup)
            bot.send_message(message.chat.id, '"удача понятие неясное, риск термин сомнительный... сомнения пораждают неясность... "', reply_markup=markup)
            ...
            if ab[34] == "100":
                markup.add(btn3)
            markup.add(btn4)
            bot.send_message(message.chat.id, "с возвращением в локацию 4)...", reply_markup=markup)
        elif message.text == "👑 взять билет 👑":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn3 = types.KeyboardButton("🎭 уйти 🎭")
            markup.add(btn3)
            bot.send_message(message.chat.id, "вы получили 👑 vip 👑", reply_markup=markup)
            bot.send_message(message.chat.id, "прирост exp+25%", reply_markup=markup)
            ab[34] = "125"
        elif message.text == "назад":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn1 = types.KeyboardButton("🌳 лес 🌲")
        ...
        # локация торговая лавка
            ...
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn4 = types.KeyboardButton("💰 продать хлам 💰")
            ...
            vvod_pokypka = message.text
            lotik = vvod_pokypka.split("_")
            if len(lotik) == 2:
                number_pokypka = int(lotik[0][1:])
                list_pok = message.text.split("_")
                count_pok = int(list_pok[number_pokypka - 1])
        ...
        # локация packs
        elif message.text == "🃏 packs 🃏":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn1 = types.KeyboardButton("🧬 characteristic pack 🧬")
            btn2 = types.KeyboardButton("назад")
            markup.add(btn1)
            markup.add(btn2)
            bot.send_message(message.chat.id, "добро пожаловать 🎲", reply_markup=markup)
            bot.send_message(message.chat.id, "здесь потратив так мало можно получить еще меньше 💲", reply_markup=markup)
            bot.send_message(message.chat.id, "как говорится удача тоже навык)", reply_markup=markup)

        elif message.text == "🧬 characteristic pack 🧬":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn1 = types.KeyboardButton("🧬 open pack 🧬")
            btn2 = types.KeyboardButton("🃏 назад 🃏")
            markup.add(btn1)
            markup.add(btn2)
            rst = "Возможный лут:" + "\n" + "☠ череп ☠ x10 (40%)"  + "\n" + "⚪ белая сфера ⚪ x1 (30%)" "\n" + "🃏 игральный камень 🃏 x1 (20%)"
            rst += "\n" + "💧 шкура водяного змея 🐍 x1 (9%)" + "\n" + "⭐⭐⭐⭐⭐" + "\n" "point x1 (1%)" + "\n" + "⭐⭐⭐⭐⭐"
            rst1 = "Стоимость:" + "\n" + "🃏 игральный камень 🃏 x1"
            bot.send_message(message.chat.id, rst, reply_markup=markup)
            bot.send_message(message.chat.id, rst1, reply_markup=markup)

        elif message.text == "🧬 open pack 🧬":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            if loot_dict["🃏 игральный камень 🃏"] != "0":
                sticker=open("tg_bot/AnimatedSticker.tgs", "rb")
                sticker1=open("tg_bot/AnimatedSticker1.tgs", "rb")
                loot_dict["🃏 игральный камень 🃏"] = str(int(loot_dict["🃏 игральный камень 🃏"]) - 1)
                x = random.randint(1, 100)
                if x <= 40:
                    bot.send_sticker(message.chat.id, sticker)
                    loot_dict["☠ череп ☠"] = str(int(loot_dict["☠ череп ☠"]) + 10)
                    bot.send_message(message.chat.id, "вы получили ☠ череп ☠ x10")
                elif x <= 70:
                    bot.send_sticker(message.chat.id, sticker)
                    loot_dict["⚪ белая сфера ⚪"] = str(int(loot_dict["⚪ белая сфера ⚪"]) + 1)
                    bot.send_message(message.chat.id, "вы получили ⚪ белая сфера ⚪ x1")
                elif x <= 90:
                    bot.send_sticker(message.chat.id, sticker)
                    loot_dict["🃏 игральный камень 🃏"] = str(int(loot_dict["🃏 игральный камень 🃏"]) + 1)
                    bot.send_message(message.chat.id, "вы получили 🃏 игральный камень 🃏 x1")
                elif x <= 99:
                    bot.send_sticker(message.chat.id, sticker)
                    loot_dict["💧 шкура водяного змея 🐍"] = str(int(loot_dict["💧 шкура водяного змея 🐍"]) + 1)
                    bot.send_message(message.chat.id, "вы получили 💧 шкура водяного змея 🐍 x1")
                else:
                    bot.send_sticker(message.chat.id, sticker1)
                    ab[20] = str(int(ab[20]) + 1)
                    bot.send_message(message.chat.id, "⭐⭐⭐ ДЖЕКПОТ ⭐⭐⭐")
                    bot.send_message(message.chat.id, "вы получили point x1")
            else:
                bot.send_message(message.chat.id, "не бесплатно 🙄🙄🙄", reply_markup=markup)
        ...
        # локация лес
            ...
            if ab[32] == "🌳 лес 🌲":
                ...
                btn2 = types.KeyboardButton("🌊 вернуться назад 🌊")
            elif ab[32] == "☯️ пустошь ☯️":
                btn2 = types.KeyboardButton("☯️ вернуться назад ☯️")
            markup.add(btn1)
            markup.add(btn3)
            markup.add(btn2)
        ...
        # локация пустошь
        elif message.text == "☯️ пустошь ☯️":
            ab[32] = "☯️ пустошь ☯️"
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn2 = types.KeyboardButton("☠ смертельная зона ☠")
            markup.add(btn2)
            btn1 = types.KeyboardButton("назад")
            markup.add(btn1)
            bot.send_message(message.chat.id, 'Вы вошли в локацию "☯️ пустошь ☯️"', reply_markup=markup)
        elif message.text == "☠ смертельная зона ☠":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            for i in based_monstrs_of_pystosh_vvod:
                if based_monstrs_of_pystosh_vvod[i]["type"] == "default":
                    btn = types.KeyboardButton(i)
                    markup.add(btn)
            btn1 = types.KeyboardButton("☯️ вернуться назад ☯️")
            markup.add(btn1)
            bot.send_message(message.chat.id, "Иногда хватает мгновения, чтобы забыть жизнь, а иногда не хватает жизни, чтобы забыть мгновение 🐺", reply_markup=markup)
        elif message.text == "☯️ вернуться назад ☯️":
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn2 = types.KeyboardButton("☠ смертельная зона ☠")
            markup.add(btn2)
            btn1 = types.KeyboardButton("назад")
        ...
        # использовать зелья
            ...
            if int(ab[17]) <= 0:
                ...
                if int(ab[17]) <= 0:
                    ...
                    bot.send_message(message.chat.id, text=f"вам начисленно: {ex}exp")
                    bot.send_message(message.chat.id, text="вы получили:")
                    # получение лута с моба
                    for i in range (1, 11):
                        d1 = "loot" + str(i)
                    ...
                    g = based_monstrs_of_prosstoles[ab[16]]["damage"]
                    sh = int(ab[33])
                    bot.send_message(message.chat.id, text=f"вам нанесли {max(int(g) - sh, 0)} урона")
                    ab[12] = str(int(ab[12]) - max(int(g) - sh, 0))
                    if int(ab[12]) < 0:
                        ab[12] = "0"
                    bot.send_message(message.chat.id, text=f"оставшиеся hp игрока: {ab[12]}")
                    # проверка hp игрока
